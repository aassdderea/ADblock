...
static BOOL _tryDirectAction(UIButton *btn) {
    @try {
        ...
        for (id t in targets) {
            // 使用 performSelector 绕过编译器检查
            NSArray *actions = [(id)btn performSelector:@selector(actionsForTarget:forControlEvents:) withObject:t withObject:@(UIControlEventTouchUpInside)];
